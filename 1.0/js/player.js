...
export class Player {
    constructor(game) {
        ...
        this.vy = 0;
        this.weight = 1;
        //JavaScript automatically creates references to all elements with IDs
        // into the global namespace, using its ID as a variable name
        //Caution: No quotation marks
        //this.image = player;
    ...
    draw(context) {
        ...
        context.drawImage(this.image, this.frameX * this.width, this.frameY * this.
            height, this.width, this.height, this.x, this.y, this.width, this.height);
    }
    onGround(){
        ...
        this.game.enemies.forEach(enemy => {
            if (!(enemy.x < this.x + this.width &&
                ...
                if (this.currentState === this.states[4] || this.currentState === this.states[5]){
                    this.game.score++;
                    this.game.floatingMessages.push(new FloatingMessages('+1',enemy.x,enemy.y,
                        130,45))
                } else {
                    this.setState(6,0);
                    this.game.score-=5;
